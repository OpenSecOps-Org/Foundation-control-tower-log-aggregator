...
This component was converted to the locked-dependency model in **v1.6.0**. The supply-chain posture below applies to v1.6.0 and every release after it. Earlier releases predate the conversion and do not carry these guarantees; per §1, the remediation for any exposure on an earlier release is to upgrade to the current release.
...
**Externally-verifiable signals.** Two third-party signals are published continuously for this component, both surfaced as badges in the repository README:

- **OpenSSF Scorecard** — automated supply-chain posture analysis covering ~18 dimensions (branch protection, signed releases, pinned dependencies, dangerous workflow patterns, etc.). Score and per-check findings published at https://scorecard.dev/viewer/?uri=github.com/OpenSecOps-Org/Foundation-control-tower-log-aggregator.
- **Daily-scan workflow status** — GitHub Actions workflow run status for the daily CVE scan (see §4 third bullet). A passing badge means the most recent scheduled scan exited clean against the published locks.

Both badges are externally readable without OpenSecOps cooperation. Combined with the Sigstore-signed release artefacts (§11) and the SLSA Build L1 in-toto provenance (§9), they constitute the customer-facing supply-chain assurance set.
...
```
https://github.com/OpenSecOps-Org/Foundation-control-tower-log-aggregator/releases/download/<VERSION>/Foundation-control-tower-log-aggregator-<VERSION>-sbom.cdx.json
```

Replace `<VERSION>` with the release tag (e.g. `v1.6.0`). The asset is attached to every release on the public OpenSecOps-Org remote.
...
That document is the authoritative cross-component reference; the per-component sections above describe how the project-wide posture applies to **Foundation-control-tower-log-aggregator** specifically.
